...
def partial_factory_citation_per_year_desc(
    txt_file_dir, dataset_file_dir, increment_size=1, metadata=False
):
...
def partial_factory_citation_desc(
    txt_file_dir, dataset_file_dir, increment_size=1, metadata=False
):
...
def partial_factory_time_bidir(
    txt_file_dir, dataset_file_dir, increment_size=1, metadata=False
):
...
def factory_citation_per_year_desc(
    txt_file_dir, dataset_file_dir, increment_size=1, metadata=False
):
    sorted_txt_files = __sort_citation_per_year_desc(txt_file_dir, metadata=metadata)
...
def __sort_citation_per_year_desc(txt_file_dir, metadata=False):
    ...
    if metadata[-6:] == ".jsonl":
        ...
        for item_id in items:
            item = items[item_id]
            txt_file_path = join(
                txt_file_dir, re.sub(r"\.pdf$", ".txt", item["pdf_file_name"])
            )
            if isfile(txt_file_path):
                citations_per_year = 0
                try:
                    order_by = int(item["google_scholar"]["cites"]) / (
                        now_year - int(item["year"]) + 1
                    )
                except:
                    order_by = 0
                pairs.append(
        ...
        for item in items:
            ...
            txt_file_path = join(
                txt_file_dir, re.sub(r"\.pdf$", ".txt", item["Document file name"])
            )
            if isfile(txt_file_path):
                pairs.append(
                    (
...
def __create_datasets(sorted_txt_files, increment_size, dataset_file_dir):
    ...
    for chunk in __get_chunks(sorted_txt_files, increment_size):
        ...
        fnm = join(
            dataset_file_dir,
            "D" + (("00000000000000000" + str(n_dataset))[-10:]) + ".txt",
        )
        fl = open(fnm, "w")
        fl.write(dataset)
        fl.close()
